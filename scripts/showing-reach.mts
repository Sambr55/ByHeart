 ...
 *
 * SEPARATE FROM showing-check.mts, which is a different question and nearly lost.
 * That one runs with no browser and no database and asserts the non-negotiables against
 * the SOURCE — that a Legend never leaves the device, that no message box appears. I
 * overwrote it with this file, because `cat >` does not ask. Two checks, two names.
 *
