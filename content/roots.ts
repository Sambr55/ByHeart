...
import { NUMBER_WORDS } from '@/content/numbers'
...
export type SetId =
  | 'numbers_1_10'
  /*
    EVERYTHING ABOVE TEN, as one group rather than as lessons.

    The picker counts to a hundred and teaches the pattern while somebody chooses — so the
    words exist and are ownable without a root per ten. See NUMBER_WORDS in
    content/numbers.ts for why they are generated rather than authored.
  */
  | 'numbers_to_100'
...
export const SETS: WordSet[] = [
  ...
  {
    id: 'numbers_to_100',
    label: 'Counting to a hundred',
    shelf: 'how_much',
    /*
      The tens and the teens — the words that are not simply the units again. Twenty-one
      through ninety-nine are these words with an `e` in the middle, which the picker
      shows rather than this list repeating eighty times.
    */
    members: [
...
export const THE_BASICS: Root[] = [
  ...
  q({
    ...
    semantic_bridge:
      'The Portuguese title is the literal one, so the whole thing arrives free — and it puts quatro next to um in the same breath, which is where the two of them belong. Casamento and funeral you can guess, which is the point: half of Portuguese is already in your head.',
    subtext: 'A number, and then a smaller one.',
    extracts: [
      { id: 'quatro', target: 'quatro', gloss: 'four', shelf: 'how_much', set: 'numbers_1_10' },
    ],
    branches: [
      { target: 'Quatro dias.', en: 'Four days.', demonstrates: ['quatro'] },
      { target: 'Quatro cafés, por favor.', en: 'Four coffees, please.', demonstrates: ['quatro'] },
      { target: 'Quatro ou cinco?', en: 'Four or five?', demonstrates: ['quatro'] },
    ],
    reinforces: ['um', 'por_favor'],
    helpers: { 'casamentos': 'weddings', 'funeral': 'funeral', 'e': 'and', 'dias': 'days', 'cafés': 'coffees', 'ou': 'or', 'cinco': 'five' },
    transfer_prompt: {
      context: 'They are asking how many of you are eating.',
      ask: 'Four, please.',
      answer: 'Quatro, por favor.',
    },
    rights_status: 'title-reference',
    starter_tags: ['numbers', 'first-day'],
  ...
  q({
    ...
    branches: [
      ...
      { target: 'Sete noites.', en: 'Seven nights.', demonstrates: ['sete'] },
      { target: 'Seis ou sete?', en: 'Six or seven?', demonstrates: ['seis', 'sete'] },
    ],
    reinforces: ['noite'],
    helpers: { 'dias': 'days', 'noites': 'nights', 'em': 'in', 'Lisboa': 'Lisbon', 'ou': 'or' },
    transfer_prompt: {
  ...
  q({
    ...
    root_display: '9 to 5',
    source: 'Nine to five.',
    target: 'Das nove às cinco.',
    semantic_bridge:
      'The two numbers everybody skips, in the one phrase where nobody forgets them. It also quietly hands you the shape Portuguese uses for any stretch of time — das X às Y — so once you have this you can say when anything opens, closes or starts.',
    subtext: 'The working day, stated the way a sign states it.',
    extracts: [
      { id: 'cinco', target: 'cinco', gloss: 'five', shelf: 'how_much', set: 'numbers_1_10' },
      { id: 'nove', target: 'nove', gloss: 'nine', shelf: 'how_much', set: 'numbers_1_10' },
    ],
    branches: [
      { target: 'Cinco minutos.', en: 'Five minutes.', demonstrates: ['cinco'] },
      { target: 'Nove euros.', en: 'Nine euros.', demonstrates: ['nove'] },
      { target: 'Das nove às cinco.', en: 'From nine to five.', demonstrates: ['nove', 'cinco'] },
    ],
    reinforces: [],
    helpers: { 'das': 'from the', 'às': 'to the', 'minutos': 'minutes', 'euros': 'euros' },
    transfer_prompt: {
      context: 'The market opens early and shuts before dinner.',
      ask: 'From five to nine.',
      answer: 'Das cinco às nove.',
    },
    rights_status: 'title-reference',
...
export const PIECES: Record<string, Piece> = (() => {
  ...
  /*
    AND THE NUMBERS, which no root teaches and every learner can reach.

    Counting stopped at ten because ten is as far as the songs go — so `cinquenta` was not
    a word this product knew, and an age could not be carried into a sentence the way
    `futebol` is. The first fix authored a root per ten, which Sam rightly called too
    literal: "we don't need to teach every number."

    The picker already teaches them. It shows the tens and the units side by side with
    their English, says them aloud, and shows fifty-six coming apart into cinquenta and
    seis — that is the lesson, and it is better than twelve songs would have been. What
    was missing was only that the word a learner tapped had nowhere to live.

    Added LAST and without overwriting, so a number a real root teaches keeps that root's
    rung, family and note: `um` stays Feist's, with its "also the word for a" attached.
    These fill the gaps around them rather than replacing anything authored.

    They are marked as coming from the basics because that is where counting is taught and
    where the picker first appears. Nothing renders them as a root — they have no line and
    no credit — and the library files them by their set like any other word.
  */
  for (const w of NUMBER_WORDS) {
    ...
    out[w.id] = {
      ...
      gloss: w.gloss,
      shelf: 'how_much',
      set: Number.isInteger(Number(w.gloss)) ? 'numbers_1_10' : 'numbers_to_100',
      family: 'the_basics',
      rung: 1,
    }
  }
